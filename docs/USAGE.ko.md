...
`TRACE` 커맨드를 전송한 뒤, daemon이 보내는 JSONL을 stdout에 그대로 출력한다.

```bash
# 실시간 이벤트 JSONL 스트림 (항상 JSONL 출력)
toki trace
```
...
- `--output-format`과 `--sink`는 report에만 적용되며, trace에는 미적용된다
- 항상 JSONL 형식으로 stdout에 출력한다
...
`trace`와 `report` 명령에서 사용 가능한 옵션. 데몬 설정은 `toki settings`로 관리한다.
...
trace에는 적용되지 않는다 — trace는 항상 JSONL을 stdout에 출력한다.
...
# HTTP 전송 (5초 timeout)
toki report --sink http://localhost:8080/v1/events

# 복수 sink (터미널 + HTTP)
toki report --sink print --sink http://localhost:8080/report
```

`--sink`은 report에만 적용된다. trace에는 미적용된다.
...
```json
{"type":"event","data":{"model":"claude-opus-4-6","source":"4de9291e","provider":"claude_code","timestamp":"2026-03-19T12:34:56.789Z","input_tokens":3,"output_tokens":14,"cache_creation_input_tokens":5139,"cache_read_input_tokens":9631,"cost_usd":0.0112}}
```
