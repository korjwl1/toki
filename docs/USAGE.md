...
```bash
# CLI 인자
clitrace daemon start --claude-root /custom/.claude --db-path /custom/clitrace.fjall

# 환경변수
CLITRACE_CLAUDE_ROOT=/custom/.claude clitrace daemon start
CLITRACE_DB_PATH=/custom/clitrace.fjall clitrace daemon start
```
...
- daemon start 시 1회 실행 + 이후 24시간 간격
- rollup은 events보다 오래 보존: events 삭제 후에도 report 가능

```bash
# 이벤트 30일, rollup 180일 보존
CLITRACE_RETENTION_DAYS=30 CLITRACE_ROLLUP_RETENTION_DAYS=180 clitrace daemon start
```
...
fn main() {
    ...
    let handle = start(config, None, Box::new(broadcast.clone()), false)
