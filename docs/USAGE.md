...
clitrace는 두 가지 모드로 동작한다:
- **`trace`**: 데몬 프로세스. cold start 후 실시간 감시
- **`report`**: one-shot 조회. TSDB 또는 JSONL에서 결과 출력 후 종료
...
2. 파싱된 이벤트를 TSDB에 저장
3. 전체 토큰 사용량 요약 출력
4. FSEvents 감시 모드 진입 (Ctrl+C로 종료)
...
```bash
clitrace trace --startup-group-by day      # 일별
clitrace trace --startup-group-by week     # 주별
clitrace trace --startup-group-by month    # 월별
clitrace trace --startup-group-by year     # 연별
clitrace trace --startup-group-by hour     # 시간별 (기존 체크포인트 필요)
```

`hour`는 증분 데이터만 출력하므로 이전에 한 번 이상 `trace`를 실행한 적이 있어야 한다.
...
clitrace trace --session-id 4de9291e

# 프로젝트 디렉토리 이름으로 필터 (서브스트링 매치)
clitrace trace --project clitrace
```

### 전체 재스캔

```bash
clitrace trace --full-rescan
```
...
```bash
# CLI 인자
clitrace trace --claude-root /custom/.claude --db-path /custom/clitrace.fjall

# 환경변수
CLITRACE_CLAUDE_ROOT=/custom/.claude clitrace trace
CLITRACE_DB_PATH=/custom/clitrace.fjall clitrace trace
```

### 단일 인스턴스 제한

동일 DB 경로에 대해 하나의 `trace` 인스턴스만 실행 가능하다.
이미 실행 중이면 `Another trace instance is already running.` 메시지와 함께 종료된다.
...
clitrace -z Europe/London trace --startup-group-by day
```
...
trace 데몬이 자동으로 오래된 데이터를 삭제한다:

| 대상 | 기본 보존 | 환경변수 |
|------|----------|----------|
| events (개별 이벤트) | 90일 | `CLITRACE_RETENTION_DAYS` |
| rollups (시간별 집계) | 365일 | `CLITRACE_ROLLUP_RETENTION_DAYS` |

- trace 시작 시 1회 실행 + 이후 24시간 간격
- rollup은 events보다 오래 보존: events 삭제 후에도 report 가능

```bash
# 이벤트 30일, rollup 180일 보존
CLITRACE_RETENTION_DAYS=30 CLITRACE_ROLLUP_RETENTION_DAYS=180 clitrace trace
```
...
CLITRACE_DEBUG=1 clitrace trace

# 레벨 2: 레벨 1 + size unchanged, no new lines 스킵 로그
CLITRACE_DEBUG=2 clitrace trace
```
