...
- Changed: §3 checkpoint to reverse-scan + xxHash3, immediate DB write; §4 select! loop; §6 two-case model
- Template Updates:
...
# Project Constitution: clitrace

## Scope & Current Target

clitrace는 AI CLI 도구들의 로컬 JSONL/JSON 로그를 감시하여
토큰 사용량과 비용을 추적하는 Rust 모듈이다.

**현재 구현 범위 (v0.1)**: **Claude Code CLI + macOS**만 대상으로 한다.
Gemini CLI, Codex CLI, Windows, Linux는 폴더 구조와 trait 인터페이스만
준비하되 구현하지 않는다.
...
- clitrace는 독립 실행보다 **모듈로 사용되는 것이 주 목적**이다.
...
- **근거**: 향후 상위 애플리케이션에서 clitrace 엔진을 라이브러리로
...
- **현재 v0.1 구현**: `providers/claude_code/` + `platform/macos/`만 구현.
  `providers/gemini/`, `providers/codex/`, `platform/windows/`,
  `platform/linux/`는 trait stub만 둔다.
...
- **아키텍처**:
  - notify 워처 스레드: FSEvents 콜백 → 채널로 이벤트 전송
  - 워커 스레드 1개: `crossbeam_channel::select!`로 이벤트 수신 + stop 시그널 +
    백업 폴링을 다중화. 파일 읽기, JSON 파싱, 즉시 DB 저장을 순차 처리.

- DB 커넥션은 워커 스레드에 소유시켜 Send 이슈를 원천 차단한다.
...
- 데이터 저장은 MUST redb(pure Rust embedded key-value DB)를 사용한다.
  외부 서비스(Redis, PostgreSQL 등) 의존은 금지한다.

- DB에 저장하는 데이터:
  - `checkpoints` 테이블: file_path(key) → {last_line_len, last_line_hash}
  - `settings` 테이블: key → value (claude_code_root 경로 등)
  - usage 이벤트 집계 데이터의 DB 영속화는 추후 결정.

- **비교 분석 근거 (SQLite vs redb)**:
  - 데이터 모델이 key-value다 (file_path → struct, key → string).
    SQL JOIN, WHERE, 집계가 필요 없다.
  - redb는 Pure Rust → C 툴체인 불필요, 크로스 컴파일 용이.
    모듈로 쓰일 때 downstream 빌드 부담 최소화.
  - 바이너리 크기: redb ~200-400KB vs rusqlite(bundled) ~1-1.5MB.
  - 수백 레코드 규모에서 성능 차이 무의미. ACID/crash safety 동등.
  - redb API가 type-safe table definition으로 더 Rustic.
