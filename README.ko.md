...
<p align="center">
  ...
  Rust로 구축. 데몬 기반. idle 5MB. 리포트 13ms. 작업을 전혀 방해하지 않습니다.
</p>
...
toki report는 데이터 크기와 무관하게 **~13ms 고정**입니다 (UDS 쿼리 → TSDB rollup 조회).
ccusage와 zzusage는 매번 모든 파일을 처음부터 다시 읽어야 합니다.
...
toki는 파싱과 **동시에** TSDB 색인까지 수행합니다. 파싱만 하는 도구보다 더 많은 일을 하면서도 더 빠릅니다.

> **참고:** 이 벤치마크는 기존 데이터 없이 처음부터 시작하는 경우를 가정한 것입니다. 실제 사용에서는 toki가 마지막 체크포인트부터 이어서 처리하기 때문에, 이전 종료 이후 새로 쌓인 데이터만 색인합니다. 따라서 일반적인 cold start는 여기 나온 수치보다 훨씬 빠릅니다.
...
- **toki** — rayon으로 모든 CPU 코어를 활용한 병렬 처리 + mmap zero-copy + 파일별 스트리밍. 최대한의 병렬성을 쓰면서도 메모리가 flat한 이유는, 파일 하나를 스트리밍으로 처리하고 바로 버리기 때문입니다. 누적이 없습니다. cold start가 끝나면 데몬은 5–11 MB, CPU ~0%로 내려갑니다. FSEvents(커널 레벨, 폴링 없음)로 파일 변경을 감시하고, Claude Code가 새 로그를 쓸 때만 깨어납니다.
- **ccusage** — 파일 하나씩 동기 블로킹으로 순차 처리합니다. 126MB라는 숫자 자체는 작아 보이지만, 매 실행마다 Node.js가 모든 파일을 하나씩 순서대로 읽는 동안 터미널이 수 초에서 수 분 동안 멈춥니다. 병렬 처리도, 증분 처리도 없습니다. 그냥 긴 블로킹 대기, 매번.
- **zzusage** — 모든 파일의 모든 이벤트를 메모리에 한꺼번에 올린 다음 처리합니다. 파싱 자체는 빠르지만, 2GB 데이터면 2.3GB RAM을 한 번에 잡아먹습니다. 더 큰 데이터셋에서는 OOM입니다.

idle 상태가 있는 건 toki뿐입니다. ccusage와 zzusage는 실행할 때마다 전체 리소스 비용을 지불합니다 — 에디터, 컴파일러, AI 에이전트 전부 같은 CPU와 메모리를 두고 경쟁합니다. toki는 그 비용을 한 번만 치르고, 5MB로 내려가서, 더 이상 방해하지 않습니다.
