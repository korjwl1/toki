...
```bash
CLITRACE_CLAUDE_ROOT=/path/to/custom/.claude cargo run --release
CLITRACE_DB_PATH=/path/to/custom.db cargo run --release
CLITRACE_DEBUG=1 cargo run --release   # 디버그 로그 (상태 전이, 이벤트, 타이밍)
CLITRACE_DEBUG=2 cargo run --release   # 레벨 1 + DB 초기화 후 강제 full cold start
CLITRACE_DEBUG=3 cargo run --release   # 레벨 1 + verbose (size unchanged, no new lines 스킵 로그)
CLITRACE_DEBUG=4 cargo run --release   # 레벨 2 + verbose (강제 cold start + 모든 스킵 로그)
```
