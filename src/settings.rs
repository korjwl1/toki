...
use cursive::views::{
    Checkbox, Dialog, DummyView, EditView, LinearLayout, PaddedView, Panel, SelectView, TextView,
};
use cursive::Cursive;
...
impl SettingsState {
    fn load() -> Self {
        ...
        SettingsState {
            ...
            daemon_sock: get(
                "daemon_sock",
                &home.join(".config/toki/daemon.sock").to_string_lossy(),
            ),
            ...
            sync_device_name: get(
                "sync_device_name",
                &crate::sync::thread::SyncConfig::default_device_name(),
            ),
        }
    }
}
...
pub fn run_settings() -> bool {
    ...
    let display_section = LinearLayout::vertical()
        ...
        .child(hint_text(
            "                        IANA format (e.g. Asia/Seoul), empty = UTC",
        ))
        .child(DummyView.fixed_height(1))
    ...
    let data_section = LinearLayout::vertical()
        ...
        .child(hint_text(
            "                        Days to keep events (0 = forever)",
        ));

    // -- Sync section (read-only except device name) --
    let sync_status = if state.sync_enabled {
        "Enabled"
    } else {
        "Disabled"
    };
    let sync_server_display = if state.sync_server.is_empty() {
        "(not configured)"
    } else {
        &state.sync_server
    };
    ...
    let sync_section = LinearLayout::vertical()
        ...
        .child(labeled_edit(
            "Device Name",
            &state.sync_device_name,
            "sync_device_name",
        ))
        .child(hint_text(
            "                        1-64 chars, no control characters",
        ))
        .child(DummyView.fixed_height(1))
        .child(hint_text("  Manage sync via CLI:"))
        .child(hint_text("    toki settings sync enable --server <host>"))
        .child(hint_text(
            "    toki settings sync disable [--delete | --keep]",
        ));
    ...
    let providers_section = LinearLayout::vertical()
        ...
        .child(DummyView.fixed_height(1))
        .child(hint_text(
            "                        Press Enter on [Change] to select providers",
        ))
        .child(cursive::views::Button::new("Change Providers", move |s| {
    ...
    let form = LinearLayout::vertical()
        ...
        .child(DummyView.fixed_height(1))
        .child(
            Panel::new(PaddedView::new(Margins::lrtb(1, 1, 1, 0), display_section))
                .title("Display"),
        )
        .child(DummyView.fixed_height(1))
...
fn save_settings(siv: &mut Cursive, restart_flag: std::sync::Arc<std::sync::atomic::AtomicBool>) {
    ...
    let output_format = siv
        .call_on_name("output_format", |v: &mut SelectView<String>| {
            v.selection()
                .map(|s| (*s).clone())
    ...
    let start_of_week = siv
        .call_on_name("start_of_week", |v: &mut SelectView<String>| {
            v.selection()
                .map(|s| (*s).clone())
    ...
    let no_cost = siv
        .call_on_name("no_cost", |v: &mut Checkbox| v.is_checked())
        .unwrap_or(false);
    ...
    let new_providers: Vec<String> = siv
        .call_on_name("providers_data", |v: &mut EditView| {
    ...
    let old_values: Vec<(String, String)> = restart_keys
        .iter()
        .map(|k| {
            (
                k.to_string(),
                crate::config::get_setting(k).unwrap_or_default(),
            )
        })
        .collect();
    ...
    let scalar_changed = old_values
        .iter()
        .any(|(k, old_v)| settings.iter().any(|(sk, sv)| sk == k && sv != old_v));
    ...
    if daemon_changed && daemon_running {
        siv.add_layer(
            ...
            .button("No", |s| {
                s.pop_layer();
                s.add_layer(Dialog::info("Run `toki daemon restart` to apply.").button(
                    "OK",
                    |s| {
                        s.pop_layer();
                        s.quit();
            ...
            LinearLayout::horizontal()
                .child(
                    Checkbox::new()
                        ...
                        .with_name(format!("popup_provider_{}", pname)),
                )
        ...
        Dialog::around(PaddedView::lrtb(1, 1, 1, 1, layout))
            ...
            .button("OK", |s| {
                ...
                for &pname in crate::providers::KNOWN_PROVIDERS {
                    ...
                    let checked = s
                        .call_on_name(&cb_name, |v: &mut Checkbox| v.is_checked())
                        .unwrap_or(false);
                    if checked {
